...
Frappe, pronounced *fra-pay*, is a full stack, batteries-included, web framework written in Python and JavaScript with MariaDB as the database. It is the framework which powers [ERPNext](https://erpnext.com). It is pretty generic and can be used to build database-driven apps.
...
Frappe has tons of features that are essential to building a modern complex app. Only the basic features are introduced here. The rest of the documentation will cover them and other advanced features with much finer detail, so make sure to read it all.